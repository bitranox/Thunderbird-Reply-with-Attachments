...
describe('confirm dialog — focus dances in a circle', () => {
  ...
  it('Shift+Tab cycles backwards', async () => {
    ...
    const [noBtn, yesBtn] = /** @type {HTMLButtonElement[]} */(Array.from(document.querySelectorAll('button')));
  ...
  it('ArrowLeft/Up move focus back; ArrowRight/Down move forward', async () => {
    ...
    key(dlg, 'ArrowRight'); expect(document.activeElement).toBe(yesBtn);
    // Left moves back to no
    key(dlg, 'ArrowLeft'); expect(document.activeElement).toBe(noBtn);
    // Up also moves back (wrap behavior)
    key(dlg, 'ArrowUp'); expect(document.activeElement).toBe(yesBtn); // from no -> up -> yes (since 2 items)
    // Finish
    noBtn.click(); await p;
  });
});
