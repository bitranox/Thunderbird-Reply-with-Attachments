...
describe('composition — storage change listeners update behavior', () => {
  ...
  beforeEach(async () => {
    vi.resetModules();
    onChanged = { addListener: vi.fn((fn) => { onChanged._ = fn; }) };
  ...
  it('enables confirmation when confirmBeforeAdd flips to true', async () => {
    ...
    browser.compose.getComposeDetails.mockResolvedValueOnce({ type: 'reply', referenceMessageId: 11 });
...
function makeBrowser({ onChanged }) {
  ...
  return { compose, messages, sessions, tabs, runtime: { onMessage: { addListener: vi.fn() } }, storage: { local: { get: vi.fn().mockResolvedValue({}) }, onChanged }, scripting };
}
