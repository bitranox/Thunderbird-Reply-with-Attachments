...
export function createBrowserMock({
  ...
  const browser = {
    ...
    windows: { create: vi.fn().mockResolvedValue({ id: 1 }), update: vi.fn().mockResolvedValue({}) },
    tabs: { onRemoved: { addListener: vi.fn() }, sendMessage: vi.fn().mockResolvedValue({ ok: true }) },
    storage: { local: { get: vi.fn().mockResolvedValue({ blacklistPatterns, confirmBeforeAdd, confirmDefaultChoice }) }, onChanged: { addListener: vi.fn() } },
    compose,
    messages,
