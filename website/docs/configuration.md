...
#### Inline pictures {#include-inline-pictures}

- There is no setting for this, and nothing to configure.
- Images embedded in the original message stay in the reply body, where Thunderbird
  puts them. The add-on leaves the body alone.
- They are not copied as file attachments, which would duplicate what the body
  already shows.
