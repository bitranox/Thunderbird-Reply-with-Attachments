...
Terminologi note: se [Glossary](glossary) for konsistente termer brugt i UI og dokumenter.

---

## Open options in Thunderbird {#open-options-in-thunderbird}

- Thunderbird → Værktøjer → Tilføjelser og temaer → find “Reply with Attachments” → Indstillinger/Optioner
...
- Keyboard: Y/J = Yes; N/Esc = No; Tab/Shift+Tab and Arrow keys cycle focus
  - Se keyboard detaljer i [Usage](usage#keyboard-shortcuts).

---

#### Blacklist (glob patterns) {#blacklist-glob-patterns}

Blacklisted files will not be added on reply automatically. Se også [Glossary](glossary) for “Blacklist (Exclude list)”.
