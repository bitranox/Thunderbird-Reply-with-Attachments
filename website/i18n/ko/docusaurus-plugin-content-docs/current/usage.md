...
- 파일 이름을 기준으로 중복이 제거되며, S/MIME 부분은 항상 건너뜁니다. 원본 메시지에 삽입된 이미지는 Thunderbird가 배치하는 답장 본문에 그대로 남아 있으며 파일로 복사되지 않습니다.
...
- 답장 감지 → 원본 첨부 파일 목록화 → S/MIME 및 삽입된 이미지 건너뛰기 → 선택적 확인 → 해당 파일 추가(중복 항목 건너뛰기).

| 파트 유형                                   | 답장에 복사됨      |
|-----------------------------------------|-------------:|
| S/MIME 서명 파일 `smime.p7s`                | 아니요          |
| S/MIME MIME 유형 (`application/pkcs7-*`)  | 아니요          |
| 메시지 본문이 `cid:`로 삽입하는 이미지                | 아니요 (본문에 있음) |
| `Content-Disposition: inline`으로 표시된 이미지 | 아니요 (본문에 있음) |
| `Content-ID`는 있지만 본문에서 참조되지 않는 이미지      | 예            |
| 파일 이름이 있는 첨부 이메일 (`message/rfc822`)     | 예            |
| 파일 이름이 있는 일반 파일 첨부                      | 예            |

이미지는 원본 메시지가 실제로 참조하거나 발신자가 명시적으로 `Content-Disposition: inline`으로 표시한 경우에만 삽입된 것으로 간주됩니다. 단순한 `Content-ID` 헤더만으로는 충분하지 않습니다. 일부 메일 클라이언트는 진짜 첨부 파일을 포함한 모든 이미지 파트에 이 헤더를 붙이므로, 그런 경우에도 여전히 복사해야 합니다.
...
- 원본 메시지가 삽입한 이미지는 파일로 복사되지 않습니다. 이미 Thunderbird가 배치한 답장 본문 안에 있습니다. [구성](configuration#include-inline-pictures)을 참조하세요.
